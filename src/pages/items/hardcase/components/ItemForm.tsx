...
import { type HardcaseItem } from "@/types/items";
...
export default function ItemForm(props: FormProps) {
    ...
    defaultValues:
      props.mode === "update"
        ? {
            ...
            monorack_code: props.data.monorack_code ?? "",
          }
        : {
            code: "",
            plan_code: "",
  ...
  const onSubmit: SubmitHandler<FormValues> = async (payload) => {
    try {
      ...
      const res =
        props.mode === "update"
          ? await updateMutate({ code: props.data.code, model: "hardcase", data })
          : await addMutate({ model: "hardcase", data });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col items-stretch gap-4">
        <div className="grid gap-2">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Monorack code</FormLabel>
                <FormControl>
                  <Input placeholder="Monorack code" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
